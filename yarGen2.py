...
def process_file(filePath):
    try:
        ...
        if size > (args.fs * 1024 * 1024):
            if args.debug:
                print("[-] File is too big - Skipping file %s" % (filePath))
            return None
        ...
        # File Info
        file_info[filePath] = {}
        file_info[filePath]["hash"] = sha256(fileData).hexdigest()
...
def parse_sample_dir(dir, notRecursive=False, generateInfo=False, onlyRelevantExtensions=False):
    files = list(get_files(dir, notRecursive))
    ...
    for result in results:
        if result:
            s_stats, o_stats, f_info = result
            string_stats.update(s_stats)
            opcode_stats.update(o_stats)
            file_info.update(f_info)
...
def score_string(string, malware_freq, goodware_freq):
    length_score = min(len(string) / 10, 1)
    entropy_score = calculate_entropy(string) / 4
    freq_score = malware_freq / (malware_freq + goodware_freq + 1)
    return (length_score * 0.3) + (entropy_score * 0.3) + (freq_score * 0.4)
...
def get_rule_strings(string_elements, opcode_elements):
    ...
    # Manejo de strings
    # Convertir string_elements a un diccionario si es una lista
    if isinstance(string_elements, list):
        string_dict = {s: {"count": 1} for s in string_elements}
    else:
        string_dict = string_elements

    # Ordenar las cadenas por puntuación
    sorted_strings = sorted(string_dict.items(), 
                            key=lambda x: score_string(x[0], x[1]["count"], good_strings_db.get(x[0], 0)), 
                            reverse=True)

    for i, (string, info) in enumerate(sorted_strings):
        if i >= strings_per_rule:
            break

        score = score_string(string, info["count"], good_strings_db.get(string, 0))
        ...
        # Determinar si es fullword
        if is_fullword(string):
            fullword = " fullword"

        # Determinar codificación
        if string.startswith("UTF16LE:"):
            string = string[8:]
            enc = " wide"

        # Comentarios adicionales
        if string in good_strings_db:
            goodware_comment = f" /* Goodware String - occured {good_strings_db[string]} times */"
        
        if args.score:
            score_comment = f" /* score: '{score:.2f}' */"

        # Formatear la regla
        if score > score_highly_specific:
            high_scoring_strings += 1
            rule_strings += f"      $x{i + 1} = \"{string}\"{fullword}{enc}{base64comment}{reversedComment}{pestudio_comment}{score_comment}{goodware_comment}{hexEncComment}\n"
        else:
            rule_strings += f"      $s{i + 1} = \"{string}\"{fullword}{enc}{base64comment}{reversedComment}{pestudio_comment}{score_comment}{goodware_comment}{hexEncComment}\n"
    ...
    if opcode_elements:
        ...
        for i, opcode in enumerate(opcode_elements):
            rule_strings += f"      $op{i + 1} = {{ {opcode} }}\n"
        opcodes_included = True

    return rule_strings, opcodes_included, string_rule_count, high_scoring_strings

def is_fullword(string):
    # Implementa la lógica para determinar si una cadena debe ser 'fullword'
    # Por ejemplo:
    return len(string) > 4 and string.isalnum()
...
def print_welcome():
    print("------------------------------------------------------------------------")
    print("                   _____            ___   ___")
    print("    __ _____ _____/ ___/__ ___     |_  | / _ \\")
    print("   / // / _ `/ __/ (_ / -_) _ \\   / __/_/ // /")
    print("   \\_, /\\_,_/_/  \\___/\\__/_//_/  /____(_)___/ ")
    print("  /___/  Yara Rule Generator")
    print("         by killuaz Version %s" % __version__)
    print("         Based on the original work by Florian Roth")
    print("   ")
    print("  Note: Rules have to be post-processed")
    print("  See this post for details: https://www.anubiscybersecurity.xyz/post/yarGen2.0")
    print("------------------------------------------------------------------------")
